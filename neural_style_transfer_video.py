...
Run the script with:
```
python neural_style_transfer.py path_to_your_base_video.jpg video_directory path_to_your_reference.jpg prefix_for_results
```
e.g.:
```
python neural_style_transfer_video.py sourcevideo/coldplay.mp4 sourcevideo styleimg/b.jpg result/result
```
```
You can also individually run the preprocessing.py, neural_style_transfer_video.py, and makemovie.py if you want to.
```
...
for file in os.listdir(directory):
    ...
    if file.endswith('.png'):
        ...
        for i in range(1):
            ...
            fname = result_prefix + '_at_iteration_%s.png' % file
            imsave(fname, img)
            end_time = time.time()
